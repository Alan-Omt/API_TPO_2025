    ...
    const loadCartItems = async () => {
      ...
      try {
        const itemsWithStock = await Promise.all(items.map(async (item) => {
          const response = await fetch(`${import.meta.env.VITE_REACT_APP_API_URL}/productos/${item.id}`);
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      await Promise.all(cartItems.map(async (item) => {
        ...
        await fetch(`${import.meta.env.VITE_REACT_APP_API_URL}/productos/${item.id}`, {
          method: 'PATCH',
          headers: {
